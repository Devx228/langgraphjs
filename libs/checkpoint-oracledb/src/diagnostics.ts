...
const assertSelectOnly = (sql: string): void => {
  if (!/^\s*(SELECT|WITH)\b/i.test(sql)) {
    throw new Error("Oracle diagnostics queries must be read-only SELECT statements.");
  }
};
...
const dataTypeMatches = (
  ...
  return expectedDataTypes.some((expected) => {
    ...
    if (normalizedExpected === "TIMESTAMP WITH TIME ZONE") {
      return normalized.includes("TIMESTAMP") && normalized.includes("WITH TIME ZONE");
    }
    return false;
  });
};
...
export const getOracleRuntimeDiagnostics = (
  ...
  const version = oracledbModule.versionString ?? String(oracledbModule.version ?? "");
  return {
...
export const inspectOracleSchema = async (
  ...
  const tableRows = await trySelectRows<{ TABLE_NAME?: string; table_name?: string }>(
    connection,
    `SELECT table_name
FROM USER_TABLES
...
export const getOracleDiagnosticsStatus = (
  ...
  const presentRequiredCount = requiredTables.filter((table) => table.exists).length;
...
const buildTableDiagnostics = ({
  ...
  const observedColumnNames = new Set(columns.map((column) => upper(column.name)));
