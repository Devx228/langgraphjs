...
class FakeDiagnosticsConnection implements OracleConnectionLike {
  ...
  async execute<RowT = FakeRow>(
    ...
    if (/SELECT v FROM/i.test(sql)) {
      const tableName = sql.match(/FROM\s+([A-Z0-9_$#_]+)/i)?.[1]?.toUpperCase();
...
function storeColumnRows(prefix: string, includeVectorTable = false): FakeRow[] {
  return [
...
function storeConstraintRows(
  ...
  return [
    ...
    ...pkRows(`${prefix}STORE`, ["NAMESPACE_PATH", "ITEM_KEY"], `${prefix}ST_PK`),
    ...(includeVectorTable
      ? pkRows(
...
describe("Oracle diagnostics", () => {
  ...
  test("uses constructor vector config for store migration expectations", async () => {
    ...
    expect(disabledDiagnostics.migrations.expectedForCurrentConfig).toEqual([0]);
